...
_NOTE_: Hindi songs with Devanagari script (e.g. Kesariya) translate well via DeepL, with occasional misdetection between Hindi (HI) and the closely related dialetics (MAI) on certain phrases. Romanised Hindi (Hinglish) is unreliable across all translation APIs as language detection fails on short Latin-script phrases. A preprocessing step for Hinglish is planned for version 2.

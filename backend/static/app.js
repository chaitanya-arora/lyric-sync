let currentTrackId = null
    ...
    async function fetchNowPlaying() {
      try {
        ...
        if (data.track_id !== currentTrackId) {
          ...
          // filter out lines where translation === original (same language)
        ...
        if (currentLyrics.length > 0) {
          const newIndex = getCurrentLineIndex(currentLyrics, progressMs)
    ...
    // ── progress tick ──
    // between API calls, increment progress locally so the bar moves smoothly
    ...
    // fetch every 5 seconds, tick every second
    fetchNowPlaying()
